...
def test_no_nan(model, tokenizer, iterations=100, device='cpu'):
    ...
    for idx in range(iterations):
        inputs = tokenizer.encode(str(idx)+text+str(idx), return_tensors="pt").to(device)
        out = model_half(input_ids=inputs, decoder_input_ids=inputs)
        if torch.isnan(out[0]).any():
            return False
        train_input_ids = tokenizer(str(idx)+train_text+str(idx), return_tensors='pt').input_ids.to(device)
        train_labels = tokenizer(str(idx)+train_label+str(idx), return_tensors='pt').input_ids.to(device)
...
def search_and_reset_layers(model, tokenizer, scale_down_factor=15, revert_old=False, device='cuda'):
