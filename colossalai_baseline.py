...
from modules.colossal_utils import gemini_zero_dpp, tensor_parallelize, get_mem_info
...
def main():
    ...
    train_dataloader = DataLoader(train_dataset, collate_fn=seq2seq_data_collator, 
        batch_size=gpc.config.BATCH_SIZE, num_workers=10, pin_memory=True)

    val_dataset = ChatGPT('dataset/sharegpt_val', tokenizer)
    test_dataloader = DataLoader(val_dataset, collate_fn=seq2seq_data_collator, 
        batch_size=gpc.config.BATCH_SIZE, num_workers=10)
    ...
    with tqdm(total=NUM_STEPS) as pbar:
        for e in range(gpc.config.NUM_EPOCHS):
            for batch in train_dataloader:
                ...
                logger.info(get_mem_info(prefix=f'[{steps}/{NUM_STEPS}] Forward '), ranks=[0])
