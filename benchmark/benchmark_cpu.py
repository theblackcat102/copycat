        ...
        for _, group in enumerate(self.param_groups):
            ...
            for p_i, p in enumerate(group["params"]):
                ...
                if target_device.type == "cpu":
                    ...
                    assert (
                        state["exp_avg"].device.type == "cpu"
                    ), "exp_avg should stay on cpu"
                    assert (
                        state["exp_avg_sq"].device.type == "cpu"
                    ), "exp_avg should stay on cpu"
                    self._pre_step(p_i, group["params"])
                    self.cpu_adam_op.adam_update(
                        self.opt_id,
                        state["step"],
                        group["lr"],
                        beta1,
                    ...
                    assert (
                        state["exp_avg"].device.type == "cuda"
                    ), "exp_avg should stay on cuda"
                    assert (
                        state["exp_avg_sq"].device.type == "cuda"
                    ), "exp_avg should stay on cuda"
