...
def gpt2_xl(checkpoint=False):
    return GPTLMModel(
        hidden_size=1600, num_layers=48, num_attention_heads=16, checkpoint=checkpoint
    )


def gpt2_8b(checkpoint=False):
    return GPTLMModel(
        hidden_size=4096, num_layers=90, num_attention_heads=16, checkpoint=checkpoint
    )


def gpt2_20b(checkpoint=False):
    return GPTLMModel(
        hidden_size=8192, num_layers=25, num_attention_heads=16, checkpoint=checkpoint
    )
        ...
        if self.prefetch > 0:
            if idx % self.prefetch == 0:
                ...
                if idx + self.prefetch < len(params):
                    for prefetch_p in params[
                        idx + self.prefetch : idx + self.prefetch * 2
                    ]:
...
def run_adam(
    model: torch.nn.Module, nvme_offload: bool, backend: str, prefetch: int, vecio: bool
):
    offloader = None
    if nvme_offload:
        offloader = DiskOffloader(".", 8, backend=backend)
    optimizer = Adam(
        model.parameters(), 1e-3, offloader=offloader, prefetch=prefetch, vecio=vecio
    )
